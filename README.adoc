...
Refactored from `[][]byte` grid with `image.Point` to flat arrays with index arithmetic.
...
Day06Part1-16      353.0µ         36.0µ       -89.80%
Day06Part2-16      72.0m          41.6m       -42.22%

              │  Original  │          Flat           │
              │ allocs/op  │ allocs/op   vs base     │
Day06Part1-16      196           2         -98.98%
Day06Part2-16      197           3         -98.48%
----

Key optimizations:

1. Flat `[]byte` grid instead of `[][]byte`
2. Integer indices instead of `image.Point`
3. Direction as int (0-3) with `[4]int` offset lookup
4. Part 1: `[]bool` visited instead of `map[image.Point]bool`
5. Part 2: path-only testing (only test positions on original path)
6. Part 2: reuse `[]bool` state array with `clear()` equivalent
...
First optimization (55ms → now 41ms with flat arrays):
- Path-only testing: ~4,900 positions vs 17,161 total
- Array-based state tracking: `stateIdx = pos*4 + direction`
- State reuse: clear and reuse visited array

**Original benchstat comparison**:
----
...
Refactored from `container/list` to slices with flat array indexing.

=== Benchstat Comparison

----
              │ container/list │          slices           │
              │     sec/op     │   sec/op    vs base       │
Day10Part1-16       656.4µ       150.9µ      -77.01%
Day10Part2-16       572.7µ       107.6µ      -81.21%

              │ container/list │          slices          │
              │     B/op       │    B/op     vs base      │
Day10Part1-16      1444.3Ki      378.1Ki     -73.82%
Day10Part2-16      1348.2Ki      324.8Ki     -75.91%

              │ container/list │         slices          │
              │   allocs/op    │ allocs/op   vs base     │
Day10Part1-16       13295         158        -98.81%
Day10Part2-16       13290         153        -98.85%
----

Key optimizations:

1. Replace `container/list` with `[]trailState` slices
2. Flatten `[][]byte` grid to `[]byte` for cache-friendly access
3. Use integer indices instead of `image.Point`
4. Pre-compute direction offsets as `[4]int{-dimX, 1, dimX, -1}`
...
Day 12 was rewritten to use `grid.go` iterators with union-find instead of DFS.

=== Grid Iterator Pattern
...
[cols="1,1,1,1,1"]
|===
| Implementation | Part 1 | Part 2 | Allocations | vs Original
...
| 2.54ms
| 5 (Part 1)
| Part 2 1.9x faster, 340x fewer allocs
|===

=== Key Insight

Grid iterators work well when:

1. Full grid scans - Iterate every cell, not random-access during DFS
2. No materialization - Iterate directly, don't collect into slices
3. Algorithm adaptation - Union-find instead of DFS for connected components
...
See `day12.go` and `grid.go` for the full implementation.
...
This is by far too wonderful to let an AI agent work on it.
...
----
              │   Original    │            Flat             │
              │    sec/op     │   sec/op     vs base        │
Day18Part1-16      173.0µ        144.0µ       -16.76%
Day18Part2-16      755.0µ        466.0µ       -38.28%

              │   Original   │           Flat            │
              │    B/op      │    B/op     vs base       │
Day18Part1-16     279Ki        298Ki        +6.81%
Day18Part2-16     820Ki        741Ki        -9.63%

              │  Original  │          Flat           │
              │ allocs/op  │ allocs/op   vs base     │
Day18Part1-16     1697         1043       -38.54%
Day18Part2-16    12690         3506       -72.37%
----
...
Note: Grid.go C4Indices was benchmarked but proved 3x slower due to neighbor table allocation overhead (16k allocs). Flat arrays with inline bounds checking are optimal for BFS pathfinding.
...
----
              │   Original    │            Flat             │
              │    sec/op     │   sec/op     vs base        │
Day20Part1-16      1.54m         0.87m        -43.51%
Day20Part2-16      46.0m         24.9m        -45.87%

              │  Original  │          Flat           │
              │ allocs/op  │ allocs/op   vs base     │
Day20Part1-16    28894          11        -99.96%
Day20Part2-16    28894          11        -99.96%
----
