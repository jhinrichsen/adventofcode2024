...
pkg: gitlab.com/jhinrichsen/adventofcode2024
cpu: AMD Ryzen 7 7840HS w/ Radeon 780M Graphics
BenchmarkDay06Part1-16    	    5900	    197549 ns/op	  663577 B/op	       2 allocs/op
BenchmarkDay06Part1-16    	    5949	    196379 ns/op	  663577 B/op	       2 allocs/op
BenchmarkDay06Part1-16    	    6105	    196933 ns/op	  663577 B/op	       2 allocs/op
BenchmarkDay06Part1-16    	    6088	    197120 ns/op	  663577 B/op	       2 allocs/op
BenchmarkDay06Part1-16    	    6140	    199124 ns/op	  663577 B/op	       2 allocs/op
----
...
That's ~2.0 seconds for part 1 with 25 blinks. The exponential stone growth dominates performance - the optimized branchless digit counting and zero-allocation parser contribute negligibly to the total runtime.


The profiling data reveals the hot path very clearly:

CPU Profile Results:

Duration: 13.74s, Total samples = 13.44s (97.83%)
flat  flat%   sum%        cum   cum%
13.35s 99.33% 99.33%     13.35s 99.33%  runtime.memmove
0.03s  0.22% 99.55%     13.38s 99.55%  slices.Insert

Key Findings:

1. runtime.memmove dominates: 99.33% of CPU time is spent in memory operations
2. slices.Insert is the bottleneck: Nearly all time (99.55% cumulative) is spent in slice insertions
3. Algorithmic functions are fast: blink(), split(), digits_branchless() barely register in the profile

Hot Path Analysis:

Primary Hot Path:
BenchmarkDay11Part1 → Day11 → blink → slices.Insert → runtime.memmove

What this means:
- The exponential stone growth creates massive slice operations
- Each slices.Insert triggers memory copy operations (memmove)
- Our optimized digit counting and splitting logic is negligible compared to memory operations
- The algorithmic optimizations we made are correct but overshadowed by the fundamental memory
bottleneck

Performance Implications:

The flame graph confirms why count-based approaches (like the reference implementations) are
superior:
- They avoid slice operations entirely
- Memory usage stays constant regardless of stone count
- CPU time scales with unique stone types, not total stone count

Our slice-based approach, while educationally valuable for finding the bug, is fundamentally
limited by memory operations rather than computational complexity.
